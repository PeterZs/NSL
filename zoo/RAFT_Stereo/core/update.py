...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args):
        ...
        if hasattr(args, 'corr_multiplier'):
            corr_multiplier = args.corr_multiplier
        else:
            corr_multiplier = 1
        cor_planes = args.corr_levels * (2*args.corr_radius + 1) * corr_multiplier
...
class BasicMultiUpdateBlockWithConf(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        super().__init__()
        self.args = args
        self.encoder = BasicMotionEncoder(args)
        ...
        self.conf = FlowHead(hidden_dims[2], hidden_dim=256, output_dim=1)
    ...
    def forward(self, net, inp, corr=None, flow=None, iter08=True, iter16=True, iter32=True, update=True):
        ...
        delta_flow = self.flow_head(net[0])
        conf = F.sigmoid(self.conf(net[0]))

        # scale mask to balence gradients
        mask = .25 * self.mask(net[0])
        return net, mask, delta_flow, conf