...
class RAFTStereoWithConf(nn.Module):
    '''FAILED!!'''
    def __init__(self, args):
        super().__init__()
        self.args = args
        ...
        self.cnet = MultiBasicEncoder(output_dim=[args.hidden_dims, context_dims], norm_fn=args.context_norm, downsample=args.n_downsample)
        self.update_block = BasicMultiUpdateBlockWithConf(self.args, hidden_dims=args.hidden_dims)
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                ...
                if self.args.n_gru_layers >= 2 and self.args.slow_fast_gru:# Update low-res GRU and mid-res GRU
                    net_list = self.update_block(net_list, inp_list, iter32=self.args.n_gru_layers==3, iter16=True, iter08=False, update=False)
                net_list, up_mask, delta_flow, conf = self.update_block(net_list, inp_list, corr, flow, iter32=self.args.n_gru_layers==3, iter16=self.args.n_gru_layers>=2)
            ...
            if up_mask is None:
                ...
                flow_up = self.upsample_flow(coords1 - coords0, up_mask)
                conf = torch.concat((conf, torch.zeros_like(conf)), dim=1)
                conf = (self.upsample_flow(conf, up_mask) / (2**self.args.n_downsample)).clamp(0., 1.)
                conf = conf[:,:1]
            flow_up = flow_up[:,:1]

            flow_predictions.append(flow_up)
            conf_predictions.append(conf)

        if test_mode:
            return coords1 - coords0, flow_up, conf

        return flow_predictions, conf_predictions
    

class RAFTStereoWithDAV2(nn.Module):
    def __init__(self, args):
        '''args中额外需包含: 
        vit_type, pretrained_path, use_bn(default: False), use_clstoken(default: False), 
        freeze_vit (default: True)
        '''
