...
from .dataset_deepsl import SimplifiedStereoDatasetWithPattern, TriSimplifiedStereoDatasetWithPattern
...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root='/home/lihanli/myproject/DeepSL/zoo/RAFT-Stereo/datasets/Middlebury', split='F'):
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        ...
        elif dataset_name.startswith('dataset_deepsl'):
            deepsl_args_path = args.deepsl_args_path
            deepsl_args = parse_yaml(deepsl_args_path)
            new_dataset = SimplifiedStereoDatasetWithPattern(deepsl_args, aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from dataset_deepsl")
    ...
    sampler = DistributedSampler(train_dataset, 1, 0, True, drop_last=True)
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, sampler=sampler, 
        pin_memory=False, shuffle=None, num_workers=num_workers, drop_last=True)

    logging.info('Training with %d image pairs' % len(train_dataset))
    return train_loader

def fetch_tri_dataloader(args):
    """ Create the data loader for the corresponding trainign set """
    ...
    train_dataset = None
    for dataset_name in args.train_datasets:
        if dataset_name.startswith('dataset_deepsl'):
            deepsl_args_path = args.deepsl_args_path
            deepsl_args = parse_yaml(deepsl_args_path)
            new_dataset = TriSimplifiedStereoDatasetWithPattern(deepsl_args, aug_params)
            logging.info(f"Adding {len(new_dataset)} samples from dataset_deepsl")
        else:
