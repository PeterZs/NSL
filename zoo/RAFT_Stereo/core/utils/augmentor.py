...
class FlowAugmentor:
    ...
    def __call__(self, img1, img2, flow, match_lr:bool=True):
        if not match_lr:
            bak_img2 = np.copy(img2)
...
class MultiInputFlowAugmentor:
    ...
    def color_transform(self, *imgs):
        ...
        # symmetric
            ...
            image_stack = np.array(self.photo_aug(Image.fromarray(image_stack)), dtype=np.uint8)
            return np.split(image_stack, len(imgs), axis=0)
    ...
    def spatial_transform(self, *imgs, flow):
        # randomly sample scale
        ht, wd = imgs[0].shape[:2]
        min_scale = np.maximum(
            (self.crop_size[0] + 8) / float(ht), 
        ...
        if np.random.rand() < self.spatial_aug_prob:
            # rescale the images
            imgs = [cv2.resize(img, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR) \
                    for img in imgs]
            flow = cv2.resize(flow, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            flow = flow * [scale_x, scale_y]
        ...
        if self.yjitter:
            shape = imgs[0].shape
            y0 = np.random.randint(2, shape[0] - self.crop_size[0] - 2)
            x0 = np.random.randint(2, shape[1] - self.crop_size[1] - 2)
            ...
            shape = imgs[0].shape
            y0 = np.random.randint(0, shape[0] - self.crop_size[0])
            x0 = np.random.randint(0, shape[1] - self.crop_size[1])
    ...
    def __call__(self, img_l, img_r, img_m, flow):
        bak_img_m = np.copy(img_m)
        img_l, img_r, img_m = self.color_transform(img_l, img_r, img_m)
        img_l, img_r, img_m = self.eraser_transform(img_l, img_r, img_m)
        img_m = bak_img_m
        img_l, img_r, img_m, flow = self.spatial_transform(img_l, img_r, img_m, flow=flow)
