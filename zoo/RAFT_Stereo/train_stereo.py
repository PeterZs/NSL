...
def train(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(data_iter): # for original datasets
        # for i_batch, idict in enumerate(tqdm(train_loader)):
            ...
            #     limg = image1[i].permute(1,2,0).cpu().numpy().astype(np.uint8)
            #     rimg = image2[i].permute(1,2,0).cpu().numpy().astype(np.uint8)
