...
from .dual_corr import DualCorrBlock1D, DualCorrBlock1D_Buggy
...
class DualCorrRAFTStereo(nn.Module):
    def __init__(self, args):
        ...
        print(f"DualCorrRaftStereo: shared fnet: {self.args.shared_fnet}")
        print(f"DualCorrRaftStereo: will use buggy tri-corr class: {hasattr(self.args, 'buggy_tri_corr') and self.args.buggy_tri_corr}")
    ...
    def forward(self, image_l, image_r, image_m, iters=12, flow_init=None, test_mode=False, corr_middle_rate=None):
        ...
        if hasattr(self.args, 'buggy_tri_corr') and self.args.buggy_tri_corr:
            corr_cls = DualCorrBlock1D_Buggy
        else:
            corr_cls = DualCorrBlock1D
        corr_fn = corr_cls(
...
class DualCorrRAFTStereoWithDAV2(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        assert not self.args.shared_backbone
        ...
        print(f"DualCorrRaftStereoWithDAV2: will use buggy tri-corr class: {hasattr(self.args, 'buggy_tri_corr') and self.args.buggy_tri_corr}")
